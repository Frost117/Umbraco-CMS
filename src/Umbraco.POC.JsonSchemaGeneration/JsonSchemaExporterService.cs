...
public class JsonSchemaExporterService
{
    ...
    public JsonSchema GenerateJsonSchema<T>()
    {
        var schema = JsonSchema.FromType<T>(
            new SystemTextJsonSchemaGeneratorSettings
            {
                ...
                SchemaProcessors =
                {
                    new MySchemaProcessor(_contentTypeInfoService, _jsonSerializerOptions),
                },
            });
        return schema;
    }
    ...
    {
        public string Generate(Type type) => UmbracoSchemaIdGenerator.Generate(type);
    }
    ...
    private class MySchemaProcessor : ISchemaProcessor
    {
        ...
        private void ApplyPolymorphicContentType(
            ...
            Func<ContentTypeInfo, List<JsonSchema>, JsonSchema> getOrCreateContentTypeSchema)
        {
            // Clear the schema
            ...
            SchemaProcessorContext context)
        {
            ...
            foreach (ContentTypePropertyInfo propertyInfo in contentType.Properties.Where(x => !x.Inherited))
            {
                JsonSchemaProperty schema = GetOrCreateSchema(propertyInfo.DeliveryApiType, context);
